...
describe("FirestoreService Failure Scenarios", () => {
  ...
  test("getDocument should return null on error", async () => {
    ...
    const result = await firestoreService.getDocument("testCollection", "testDoc");
    expect(result).toBeNull();
    expect(console.error).toHaveBeenCalledWith("Error getting document:", expect.any(Error));
  });

  test("setDocument should throw an error on failure", async () => {
    mockedSetDoc.mockRejectedValue(new Error("Failed to set document"));
    await expect(firestoreService.setDocument("testCollection", "testDoc", {})).rejects.toThrow("Failed to set document");
    expect(console.error).toHaveBeenCalledWith("Error setting document:", expect.any(Error));
  });
  ...
  test("updateDocument should throw an error on failure", async () => {
    ...
    await expect(firestoreService.updateDocument("testCollection", "testDoc", {})).rejects.toThrow("Failed to update document");
  ...
  test("deleteDocument should throw an error on failure", async () => {
    ...
    await expect(firestoreService.deleteDocument("testCollection", "testDoc")).rejects.toThrow("Failed to delete document");
  ...
  test("getCollection should return an empty array on error", async () => {
    ...
    expect(console.error).toHaveBeenCalledWith("Error getting collection:", expect.any(Error));
  });
});